...
class ForegroundVoiceService : Service() {
    ...
    // Tracks whether the current session was started by mic button tap vs wake word.
    // After a mic-button session, we stay connected (don't return to wake-word mode)
    // so the user can tap again immediately without waiting for a new WS handshake.
    private var activatedByMicButton = false
    ...
    private fun startActiveConversation() {
        // Skip voice auth for mic-button sessions — VoiceIdentifier opens its
        // own AudioRecord for 2 seconds which conflicts with AudioRecorder that
        // starts right after. Mic button is always treated as owner (conversation
        // mode). Voice auth only matters for wake-word sessions (phone commands).
        if (activatedByMicButton) {
            Logger.d(TAG, "Mic-button session — skipping voice auth, starting recording directly")
            vad.reset()
            ...
            VoiceStateManager.setListening()
            sendBroadcast(Intent("MAYA_RESPONSE").putExtra("text", "🎤 Listening..."))
            return
        }

        // Wake-word session: authenticate voice before recording command
    ...
    private fun connectGemini(apiKey: String) {
        ...
        connectionManager = ConnectionManager(
            ...
            onTurnComplete = {
                audioRecorder.stop()
                if (activatedByMicButton) {
                    // Mic button session — stay connected and go idle so the
                    // user can tap again immediately without a new WS handshake.
                    Logger.d(TAG, "Turn complete (mic-button session) — staying connected, ready for next tap")
                    activatedByMicButton = false
                    currentMode = Mode.WAKE_WORD
                    VoiceStateManager.setIdle()
                } else {
                    // Wake-word session — return to wake-word-only mode.
                    Logger.d(TAG, "Turn complete (wake-word session) — returning to wake-word mode")
                    scope.launch {
    ...
    fun toggleRecording(): Boolean {
        ...
        return if (audioRecorder.isActive()) {
            ...
            wakeWordDetector?.stop()
            currentMode = Mode.ACTIVE
            activatedByMicButton = true  // track that this session came from mic button
            vad.reset()
            VoiceStateManager.setListening()
